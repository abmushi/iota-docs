...
## Address generation
　Note that every address has **81 tryte length**. However, private key and digest are of different length, and length depends on the level of `security`. It implies address is generated by the mechanism with which any length of digest(pricisely must be multiples of 81) produces an address with length of 81 trytes.
